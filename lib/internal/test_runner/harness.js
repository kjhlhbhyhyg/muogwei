...
function createTestTree({ coverage, coverageExcludeGlobs, coverageIncludeGlobs, ...options } = kEmptyObject) {
  const coverageOpts = coverage == null ? undefined : { __proto__: null, coverage, coverageExcludeGlobs, coverageIncludeGlobs };
  return setup(new Test({ __proto__: null, ...options, name: '<root>' }), coverageOpts);
}
...
function setup(root, coverageOpts) {
  ...
  const coverage = configureCoverage(root, coverageOpts ?? globalOptions);
