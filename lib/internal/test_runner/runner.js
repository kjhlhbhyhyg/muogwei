...
function run(options = kEmptyObject) {
  ...
  if (coverageExcludeGlobs != null) {
    if (!coverage) {
      throw new ERR_INVALID_ARG_VALUE('options.coverageExcludeGlobs', coverageExcludeGlobs, 'is not supported when not in coverage mode');
    }
  ...
  if (coverageIncludeGlobs != null) {
    if (!coverage) {
      throw new ERR_INVALID_ARG_VALUE('options.coverageIncludeGlobs', coverageIncludeGlobs, 'is not supported when not in coverage mode');
    }
  ...
  const root = createTestTree({ __proto__: null, concurrency, timeout, signal, coverage, coverageExcludeGlobs, coverageIncludeGlobs });
